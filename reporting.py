"""reporting.py — Fiber EUR v1.5 Telegram Performance Reports
Fiber EUR v1.5 reporting — three scheduled reports.
...
def _stats(trades: list) -> dict:
    ...
    # R-multiple (uses estimated_risk_usd stored on the trade record)
    r_vals = []
    for t in trades:
...
def send_monthly_csv_export() -> None:
    ...
    try:
        ...
        caption = (
            f"📊 Fiber EUR v1.5 — Cumulative Trade Log\n"
        ...
        with open(tmp_path, "rb") as fh:
            r = _req.post(
                ...
                timeout=int(_load_settings_rep().get("http_timeout_sec", 30)),
            )
...
def send_weekly_export() -> None:
    ...
    try:
        ...
        fieldnames = ["date_sgt", "time_sgt", "session", "direction", "score",
                      "entry_price", "result", "pl_usd", "duration_min"]
        buf    = _io.StringIO()
        ...
        caption = (
            f"📎 Fiber EUR v1.5 — Weekly Trade Log\n"
        ...
        with open(tmp_path, "rb") as fh:
            r = _req.post(url, data={"chat_id": chat_id, "caption": caption},
                          files={"document": (filename, fh, "text/csv")}, timeout=int(_load_settings_rep().get("http_timeout_sec", 30)))
