...
Size:      Dynamic risk-based sizing ($75 risk/trade by default)
SL:        15 pips
...
Protection:
  Chaos filter    — skip if daily range > 150 pips
            ...
            if data:
                ...
                try:
                    ...
                    history.append({
                        ...
                        "estimated_risk_usd": abs(state.get("last_risk_usd", state.get("last_sl_sgd", 0))),
                    })
...
def run_bot(state: dict) -> None:
    ...
    MAX_TRADES_DAY   = int(settings.get("max_trades_day", 4))
        ...
        try:
            ...
            if mins >= MAX_DURATION:
                ...
                if result.get("success"):
                    ...
                    log.warning("%s: timeout close failed — %s", name, result.get("error", ""))
        except Exception as e:
            state.get("open_times", {}).pop(name, None)
    ...
    if _daily_cap > 0 and _daily_used >= _daily_cap:
        log.info("RISK CAP: %.2f/%.2f used — done for the day", _daily_used, _daily_cap)
        return
    ...
    for name, cfg in ASSETS.items():
        ...
        risk_plan = build_risk_plan(settings, sl_pips=use_sl, price=price, account_summary=account_summary)
        ...
        if result_order["success"]:
            ...
            state["last_risk_usd"]           = risk_plan.risk_amount
